...
#                break
    ...
    def DiscountedReturn(self, occlusions, predatorHome, knowledge,occlusionInd, predatorInd, simulationDirectory, visualRange=None):
        ...
        if visualRange is None:
            predatorDirectory = occlusionDirectory + '/predator_%d'%(predatorInd)
            Path(predatorDirectory).mkdir(parents=True, exist_ok=True)
            summaryFile = simulationDirectory + '/Occlusion_%d/Predator_%d/Summary.csv' % (occlusionInd, predatorInd)
        else:
            predatorDirectory = occlusionDirectory + '/predator_%d/VisualRange_%d'%(predatorInd, visualRange)
            Path(predatorDirectory).mkdir(parents=True, exist_ok=True)
            summaryFile = simulationDirectory + '/Occlusion_%d/Predator_%d/VisualRange_%d/Summary.csv' % (occlusionInd, predatorInd, visualRange)
        ...
        for depth in ExperimentParams.Depth:
            if depth == 5000:
                if visualRange is None:
                    ...
                    directory = simulationDirectory + '/Occlusion_%d/Predator_%d/VisualRange_%d/Depth_%d' % (occlusionInd, predatorInd, visualRange, depth)
                    Path(directory).mkdir(parents=True, exist_ok=True)
            else:
